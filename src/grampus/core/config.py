"""GrampusConfig — hierarchical configuration loaded from env vars and YAML."""
...
class VectorStoreConfig(BaseModel):
    ...
    pinecone_namespace: str = "grampus"
    ...
    weaviate_collection: str = "GrampusMemory"

    # Qdrant
    qdrant_url: str = "http://localhost:6333"
    qdrant_api_key: SecretStr | None = None
    qdrant_collection: str = "grampus_memory"
...
class ModelConfig(BaseSettings):
    ...
    model_config = {"env_prefix": "GRAMPUS_MODEL__", "extra": "ignore"}
...
class MemoryConfig(BaseSettings):
    ...
    model_config = {"env_prefix": "GRAMPUS_MEMORY__", "extra": "ignore"}
...
class SafetyConfig(BaseSettings):
    ...
    model_config = {"env_prefix": "GRAMPUS_SAFETY__", "extra": "ignore"}
...
class DaprConfig(BaseSettings):
    ...
    model_config = {"env_prefix": "GRAMPUS_DAPR__", "extra": "ignore"}
...
class ObservabilityConfig(BaseSettings):
    ...
    model_config = {"env_prefix": "GRAMPUS_OBSERVABILITY__", "extra": "ignore"}


def _yaml_path() -> str | None:
    """Resolve the YAML config file path from env or default locations."""
    env_path = os.environ.get("GRAMPUS_CONFIG_FILE")
    if env_path:
        return env_path
    for candidate in ("grampus.yaml", "grampus.yml"):
        if Path(candidate).exists():
            return candidate
    return None


class GrampusConfig(BaseSettings):
    """Top-level application configuration.

    Loads from (in priority order, highest first):
      1. Environment variables (GRAMPUS_ prefix)
      2. YAML file at GRAMPUS_CONFIG_FILE / grampus.yaml / _config_file kwarg
      3. Coded defaults
    """
    ...
    model_config = {"env_prefix": "GRAMPUS_", "env_nested_delimiter": "__", "extra": "ignore"}

    def __init__(self, _config_file: str | None = None, **data: Any) -> None:
        if _config_file is not None:
            GrampusConfig._active_yaml_path = _config_file
        try:
            super().__init__(**data)
        finally:
            GrampusConfig._active_yaml_path = None
