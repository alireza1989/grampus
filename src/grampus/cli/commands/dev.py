"""grampus dev — watch-mode development server."""
...
def _print_config_summary(cfg: Any) -> None:
    ...
    name = getattr(agent_cfg, "name", "grampus-agent") if agent_cfg else "grampus-agent"
...
def _run_dev_loop(cfg: Any, *, config_path: str) -> None:
    ...
    Args:
        cfg: Loaded GrampusConfig.
        config_path: Path to grampus.yaml for file watching.
    """
    watch_dir = str(Path(config_path).parent)
    click.echo(f"[grampus dev] Watching {watch_dir} for changes...")
    click.echo("[grampus dev] Press Ctrl+C to stop.\n")
...
async def _watch_with_watchfiles(watch_dir: str) -> None:
    ...
    async for changes in _awatch(watch_dir):
        for _, path in changes:
            click.echo(f"[grampus dev] File changed: {path} — restart required")


async def _watch_with_polling(watch_dir: str) -> None:
    """Minimal polling loop when watchfiles is not installed."""
    click.echo("[grampus dev] (watchfiles not installed — polling every 2s)")
    while True:
