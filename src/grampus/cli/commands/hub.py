"""CLI commands for the Grampus Hub template gallery."""
...
@click.group()
def hub() -> None:
    """Browse, pull, and validate agent templates from the Grampus Hub."""
...
def pull_template(
    ...
    click.echo(f"  grampus run {result.entry_point.name}")


# ---------------------------------------------------------------------------
# grampus hub validate
# ---------------------------------------------------------------------------
...
def validate_template(template_dir: Path) -> None:
    """Validate a local grampus-template.yaml and its referenced files."""
    errors: list[str] = []
    ...
    if not manifest_path.exists():
        click.echo("  ✗ grampus-template.yaml — not found")
        sys.exit(1)

    try:
        manifest = TemplateManifest.from_yaml(manifest_path)
        click.echo("  ✓ grampus-template.yaml — valid")
    except Exception as exc:
        click.echo(f"  ✗ grampus-template.yaml — invalid: {exc}")
        sys.exit(1)
...
def push_template(template_dir: Path) -> None:
    """Submit a template to the Grampus Hub (opens a GitHub PR)."""
    click.echo("To share your template with the community:\n")
    click.echo("1. Fork https://github.com/grampus-ai/grampus-templates")
    click.echo("2. Copy your template directory into the fork")
    click.echo("3. Run: grampus hub validate <your-template-dir>")
    click.echo("4. Open a Pull Request at:")
    click.echo("   https://github.com/grampus-ai/grampus-templates/pulls")
    click.echo()
    click.echo("Guidelines:")
    click.echo("  - Template must pass `grampus hub validate`")
