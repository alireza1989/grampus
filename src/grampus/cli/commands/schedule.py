"""grampus schedule — manage scheduled agent runs via Dapr Jobs."""
...
from grampus.cli.commands._utils import load_config, print_table
from grampus.core.errors import ConfigError, DaprJobsError
...
async def _create_async(
    ...
    from grampus.dapr.schedule_store import ScheduleConfig
    ...
    try:
        ...
        click.echo(f"Error: {exc}")
        click.echo("  Hint: Ensure Dapr sidecar is running: dapr run --app-id grampus ...")
        sys.exit(1)
...
async def _list_async(config_path: str) -> None:
    ...
    click.echo(
        ...
        "Use 'grampus schedule get <name>' or check your Dapr dashboard."
    )
...
def schedule_trigger(name: str, server: str) -> None:
    """Manually trigger a scheduled job (calls running grampus serve instance)."""
...
async def _trigger_async(name: str, server: str) -> None:
    ...
    try:
        ...
        click.echo(f"Error: Cannot connect to {server}. Is 'grampus serve' running?")
        sys.exit(1)
