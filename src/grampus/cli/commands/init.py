"""grampus init — scaffold a new Nexus agent project."""
...
def create_agent_def() -> AgentDefinition:
    \"\"\"Return the agent definition loaded from grampus.yaml.\"\"\"
...
spec:
  ...
  metadata:
    - name: connectionString
      value: "host=localhost user=grampus password=grampus dbname=grampus port=5432 sslmode=disable"
    - name: schema
      value: "public"
    - name: tableName
      value: "grampus_state"
...
GRAMPUS_MODEL__ANTHROPIC_API_KEY=your-anthropic-api-key
GRAMPUS_MODEL__OPENAI_API_KEY=your-openai-api-key
GRAMPUS_MODEL__DEFAULT_MODEL=claude-sonnet-4-6
"""
...
grampus run agent.py --input "Hello, agent!"
```

## Running evals

```bash
grampus eval eval_suite.py
```
"""
...
@click.command("init")
@click.option("--name", default="grampus-agent", show_default=True, help="Project name.")
@click.option(
    "--template",
