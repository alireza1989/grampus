"""grampus run — execute an agent from a Python file."""
...
from grampus.cli.commands._utils import load_config, load_module, require_function
from grampus.core.errors import ConfigError
from grampus.core.logging import get_logger
from grampus.core.types import AgentDefinition, ExecutionResult, StreamEventType

_log = get_logger(__name__)


def _print_grampus_error(exc: Exception) -> None:
...
@click.option(
    "--config",
    "config_path",
    default="grampus.yaml",
    show_default=True,
    help="Path to grampus.yaml configuration file.",
)
...
def run(
    ...
    sid = session_id or f"grampus-{uuid.uuid4().hex[:8]}"
...
def _build_runner(module: Any, cfg: Any) -> Any:
    ...
    Args:
        ...
        cfg: Loaded GrampusConfig (reserved for future dependency injection).
...
def _resolve_agent_def(module: Any, cfg: Any) -> AgentDefinition:
    """Resolve the AgentDefinition from the module or fall back to grampus.yaml.

    Args:
        module: Dynamically loaded agent module.
        cfg: Loaded GrampusConfig used as fallback.
...
def _agent_def_from_config(cfg: Any) -> AgentDefinition:
    """Build a minimal AgentDefinition from GrampusConfig.

    Args:
        cfg: GrampusConfig with agent sub-settings stored in a grampus.yaml file.
    ...
    if agent_cfg is not None:
        return AgentDefinition(
            name=getattr(agent_cfg, "name", "grampus-agent"),
    ...
    return AgentDefinition(name="grampus-agent", model=cfg.model.default_model)
