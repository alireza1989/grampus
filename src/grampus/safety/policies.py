...
class GrampusSafetyPolicy(BaseModel):
...
class PolicyLoader:
    ...
    @staticmethod
    def load(path: str | None = None) -> GrampusSafetyPolicy:
        ...
        if not file_path.exists():
            raise ConfigError(
                ...
                hint="Check the path in grampus.yaml under safety.policy_file, or run 'grampus init' to generate a default policy.",
            )
        ...
        try:
            return GrampusSafetyPolicy.model_validate(raw or {})
        except Exception as exc:
            raise ConfigError(
    ...
    @staticmethod
    def build_pipeline(policy: GrampusSafetyPolicy, *, agent_id: str) -> SafetyPipeline:
        """Construct a fully configured SafetyPipeline from a loaded policy.

        Args:
            policy: A validated GrampusSafetyPolicy.
...
def load_safety_policy(path: str) -> GrampusSafetyPolicy:
    """Convenience wrapper: load a YAML policy and return GrampusSafetyPolicy.

    Accepts both the full GrampusSafetyPolicy field names AND the shorthand
    ...
    # Translate shorthand structure → GrampusSafetyPolicy field names
    ...
    # Allow full GrampusSafetyPolicy fields to pass through as-is
    ...
    return GrampusSafetyPolicy.model_validate(normalised)
