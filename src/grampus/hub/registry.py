...
DEFAULT_REGISTRY_URL = (
    "https://raw.githubusercontent.com/grampus-ai/grampus-templates/main/registry.json"
)
...
class TemplateRegistry:
    ...
    def fetch_manifest(self, name: str) -> TemplateManifest:
        """Fetch and parse the full grampus-template.yaml for a template.

        Built-in templates are loaded directly from the package; remote
        templates are fetched via HTTP.
        """
        builtin = _TEMPLATES_DIR / name / "grampus-template.yaml"
        if builtin.exists():
    ...
    def _load_builtin_templates(self) -> list[TemplateIndexEntry]:
        """Scan the bundled templates/ directory for grampus-template.yaml files."""
        entries: list[TemplateIndexEntry] = []
        if not _TEMPLATES_DIR.is_dir():
            return entries
        for manifest_path in sorted(_TEMPLATES_DIR.glob("*/grampus-template.yaml")):
            try:
