...
from grampus.tools.library.document_types import DocumentChunk, DocumentMetadata, ParsedDocument
...
async def read_pdf(path: str | Path, chunker: DocumentChunker) -> ParsedDocument:
    ...
    try:
        ...
        try:
            ...
            raise ToolError(
                "PDF support requires: pip install 'grampus-ai[documents]'",
...
async def read_docx(path: str | Path, chunker: DocumentChunker) -> ParsedDocument:
    ...
    try:
        ...
        raise ToolError(
            "DOCX support requires: pip install 'grampus-ai[documents]'",
            code="MISSING_DEPENDENCY",
        ) from None
...
async def read_excel(path: str | Path, chunker: DocumentChunker) -> ParsedDocument:
    ...
    try:
        ...
        raise ToolError(
            "Excel support requires: pip install 'grampus-ai[documents]'",
            code="MISSING_DEPENDENCY",
        ) from None
