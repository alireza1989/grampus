...
LIBRARY_REGISTRY.register(
    ...
    description=(
        ...
        "Requires: pip install 'grampus-ai[documents]'."
    ),
    parameters=[
        ToolParameter(
...
LIBRARY_REGISTRY.register(
    sql_query,
    name="sql_query",
    description="Execute a read-only SELECT query against a database. Requires grampus-ai[sql] (sqlalchemy + aiosqlite).",
    parameters=[
        ToolParameter(
            name="query",
            type="string",
...
LIBRARY_REGISTRY.register(
    ...
    parameters=[
        ToolParameter(
            name="path",
            type="string",
            description="Path to the .py file to type-check.",
            required=True,
        ),
    ],
)
