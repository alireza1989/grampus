...
class GrampusMetrics:
    ...
    def to_prometheus_text(self) -> str:
        ...
        lbl = self._label
        _emit_counter(
            lines, "grampus_total_tokens", "Total tokens consumed", lbl, self._total_tokens
        )
        _emit_counter(lines, "grampus_total_cost_usd", "Total cost in USD", lbl, self._total_cost)
        _emit_counter(
            lines,
        ...
        _emit_counter(
            lines, "grampus_total_errors", "Total errors recorded", lbl, self._total_errors
        )
        _emit_counter(
            lines, "grampus_llm_call_count", "Total LLM calls made", lbl, self._llm_call_count
        )

        lines.append("# HELP grampus_active_agents Currently active agents")
        lines.append("# TYPE grampus_active_agents gauge")
        lines.append(f"grampus_active_agents{lbl} {self._active_agents}")

        _emit_histogram(
            lines, "grampus_llm_latency_ms", "LLM call latency in ms", lbl, self._llm_latency
        )
        _emit_histogram(
            lines, "grampus_tool_latency_ms", "Tool call latency in ms", lbl, self._tool_latency
        )
        return "\n".join(lines) + "\n"
