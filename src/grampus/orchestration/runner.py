...
from grampus.core.errors import OrchestrationError, SafetyError, UncertaintyError
...
from grampus.orchestration.handoff import HandoffContext, HandoffExecutor, HandoffRequest
...
if TYPE_CHECKING:
    ...
    from grampus.orchestration.cost_tracker import CostSummary, CostTracker
...
class AgentRunner:
    ...
    def __init__(
        ...
        *,
        ...
        grampus_metrics: GrampusMetrics | None = None,
        ...
        *,
        ...
        try:
            for i in range(self._config.max_iterations):
                ...
                _llm_ctx = None
                if self._plugins:
                    from grampus.plugins.types import HookBlockedError, LLMCallContext
            ...
            if self._plugins:
                with contextlib.suppress(Exception):
                    from grampus.plugins.types import ErrorContext
        ...
        if self._plugins:
            with contextlib.suppress(Exception):
                from grampus.plugins.types import AgentEndContext
    ...
    async def _execute_tool_calls(
        ...
        for tc in tool_calls:
            if tc.name == "human_input":
                ...
                _tc_exec = tc
                if self._plugins:
                    from grampus.plugins.types import HookBlockedError, ToolCallContext
                ...
                if self._plugins and _tool_ctx is not None:
                    with contextlib.suppress(Exception):
                        from grampus.plugins.types import ToolResultContext
