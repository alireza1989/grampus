...
_INSTALL_HINT = "Install Qdrant support: pip install 'grampus-ai[qdrant]'"
...
def _to_qdrant_uuid(grampus_id: str) -> str:
...
class QdrantVectorStore(VectorStore):
    ...
    in the point payload under ``_grampus_id``.
    ...
    def __init__(
        ...
        collection_name: str = "grampus_memory",
        _client: Any = None,
    ) -> None:
        self._url = url
    ...
    async def upsert(self, records: list[VectorRecord]) -> None:
        ...
        points = [
            models.PointStruct(
                ...
                vector=r.vector,
                payload={**r.payload, "_grampus_id": r.id},
            )
            for r in records
        ]
        await client.upsert(
    ...
    async def search(
        ...
        return [
            VectorSearchResult(
                id=str(p.payload.get("_grampus_id", p.id)),
                score=float(p.score),
