...
from grampus.memory.vector.base import VectorRecord, VectorSearchResult, VectorStore

_NS = _uuid.UUID("6ba7b810-9dad-11d1-80b4-00c04fd430c8")  # uuid.NAMESPACE_URL
_INSTALL_HINT = "Install Weaviate support: pip install 'grampus-ai[weaviate]'"


def _to_weaviate_uuid(grampus_id: str) -> str:
    """Convert a Nexus string ID to a deterministic UUID5."""
    return str(_uuid.uuid5(_NS, grampus_id))
...
class WeaviateVectorStore(VectorStore):
    ...
    payload under ``_grampus_id`` so it can be recovered on search.
    ...
    def __init__(
        ...
        collection_name: str = "GrampusMemory",
        _client: Any = None,
    ) -> None:
        self._host = host
        self._port = port
        ...
        async with client_ctx as client:
            ...
            data_objects = [
                DataObject(
                    ...
                    properties={**r.payload, "_grampus_id": r.id},
                    vector=r.vector,
                )
                for r in records
            ]
            ...
            if response.errors:
                for idx_str, _err in response.errors.items():
                    ...
                    await col.data.replace(
                        ...
                        properties={**record.payload, "_grampus_id": record.id},
    ...
    async def search(
        ...
        for obj in response.objects:
            grampus_id = obj.properties.get("_grampus_id", "")
    ...
    async def delete(self, ids: list[str]) -> None:
        ...
        async with client_ctx as client:
            col = client.collections.get(self._collection_name)
            for grampus_id in ids:
                with contextlib.suppress(Exception):
                    await col.data.delete_by_id(_to_weaviate_uuid(grampus_id))
