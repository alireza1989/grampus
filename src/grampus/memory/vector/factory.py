...
from grampus.memory.vector.base import VectorStore, VectorStoreType
...
def create_vector_store(config: Any, state_store: Any = None) -> VectorStore:
    ...
    """
    from grampus.memory.vector.pgvector import PgVectorStore  # noqa: PLC0415
    from grampus.memory.vector.pinecone import PineconeVectorStore  # noqa: PLC0415
    from grampus.memory.vector.qdrant import QdrantVectorStore  # noqa: PLC0415
    from grampus.memory.vector.weaviate import WeaviateVectorStore  # noqa: PLC0415
        ...
        case VectorStoreType.PINECONE:
            if not config.pinecone_api_key:
                raise ConfigError(
                    ...
                    "Set GRAMPUS_MEMORY__VECTOR_STORE__PINECONE_API_KEY.",
                    code="config_missing_pinecone_api_key",
                )
            if not config.pinecone_index_host:
                raise ConfigError(
                    "Pinecone index host is required. "
                    "Set GRAMPUS_MEMORY__VECTOR_STORE__PINECONE_INDEX_HOST.",
