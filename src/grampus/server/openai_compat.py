...
from grampus.core.types import AgentDefinition, StreamEventType, TokenUsage
...
class OAIChatResponse(BaseModel):
    ...
    usage: OAIUsage
    system_fingerprint: str = "grampus-v0.1"
...
def _grampus_usage_to_oai(token_usage: TokenUsage) -> OAIUsage:
    return OAIUsage(
...
async def _blocking_response(
    ...
    response = OAIChatResponse(
        ...
        usage=_grampus_usage_to_oai(result.token_usage),
    )
            ...
            async for event in runner.stream(effective_def, user_input, session_id=session_id):
                if event.event_type == StreamEventType.TOKEN and event.chunk and event.chunk.delta:
                    ...
                    if event.chunk and event.chunk.token_usage:
                        accumulated_usage = _grampus_usage_to_oai(event.chunk.token_usage)
