...
try:
    ...
    raise ImportError("Install server deps: pip install grampus-ai[server]") from _exc

from grampus.core.logging import get_logger
from grampus.core.types import AgentDefinition, Role, StreamEvent, StreamEventType
...
from grampus.observability.events import AgentEvent, EventLog
from grampus.observability.metrics import GrampusMetrics
from grampus.server._hitl_html import UI_HTML
...
async def _run_scheduled_job(
    ...
    from grampus.core.logging import get_logger as _get_logger
...
def build_a2a_router(
    ...
    Mounts:
      ...
      - /a2a/agents                   (Grampus-specific: list registered agents)

    When ``a2a_executor`` is None, the ``/a2a`` endpoint returns 503.

    Args:
        agent_def: The AgentDefinition this server exposes.
        a2a_executor: Configured GrampusA2AExecutor, or None.
        a2a_task_store: Configured GrampusTaskStore, or None.
    ...
    from grampus.orchestration.a2a.auth import make_api_key_verifier
    ...
    # /a2a/agents — list registered local agents (Grampus-specific, keep from D3)
...
def create_router(has_memory: bool) -> APIRouter:
    ...
    async def metrics_endpoint(request: Request) -> str:
        ...
        grampus_metrics: GrampusMetrics | None = getattr(request.app.state, "grampus_metrics", None)
        if grampus_metrics is None:
            return "# No metrics collector configured\n"
        return grampus_metrics.to_prometheus_text()
    ...
    async def trigger_webhook(
        ...
        sig_header = request.headers.get("X-Grampus-Signature")
