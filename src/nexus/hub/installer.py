...
class TemplateInstaller:
    ...
    def install_local(
        ...
        """Install from a local directory containing nexus-template.yaml."""
    ...
    def _download_template(self, entry: TemplateIndexEntry, dest: Path) -> Path:
        ...
        if len(subdirs) == 1 and (subdirs[0] / "nexus-template.yaml").exists():
            return subdirs[0]
        if (dest / "nexus-template.yaml").exists():
            return dest
    ...
    def _render_and_write(
        ...
        if manifest.files:
            ...
            file_list = [
                ...
                if p.is_file() and p.name != "nexus-template.yaml"
            ]
