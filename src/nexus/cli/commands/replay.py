"""nexus replay — step through a past agent session from its event log."""
...
from nexus.observability.events import AgentEvent, EventType
...
def replay(
    ...
    SESSION_ID is the session identifier printed by `nexus run`.
    Requires a state store configured in nexus.yaml.
    """
...
async def _replay_async(
    ...
    resolved_agent_id = (
        agent_id or getattr(getattr(cfg, "agent", None), "name", None) or "nexus-agent"
    )
    ...
    try:
        ...
        state_store: Any = DaprStateStore(gateway=gateway, store_name=store_name, namespace="nexus")
