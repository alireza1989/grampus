"""Tests for grampus.jupyter.display — HTML/rich rendering helpers."""
...
class TestRenderResult:
    def test_render_result_returns_string_without_ipython(self) -> None:
        with patch("grampus.jupyter.display._ipython_available", return_value=False):
...
class TestRenderToolCall:
    ...
    def test_render_tool_call_contains_args_preview(self) -> None:
        with patch("grampus.jupyter.display._ipython_available", return_value=False):
            result = render_tool_call("search_web", {"query": "grampus"})
        assert "grampus" in result
