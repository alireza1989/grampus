...
def _write_agent_py(path: Path, *, has_runner: bool = True, has_agent_def: bool = True) -> Path:
    ...
    if has_runner:
        ...
        lines.append("from grampus.orchestration.runner import AgentRunner, RunnerConfig")
        lines.append("")
        lines.append("def create_runner():")
        lines.append("    return MagicMock()")
    if has_agent_def:
        lines.append("")
        lines.append("from grampus.core.types import AgentDefinition")
